...
export default async function InrToUsdSubscriptionConverter(creditOfRazorpay, period) {
    try {
        ...
        const matchDollar = creditCostMappings.find(([credit]) => {
            if (period === 'is_monthly' || period === 'monthly') {
                return credit === creditOfRazorpay; // For monthly, match directly
            } else {
                return credit * 12 === creditOfRazorpay; // For annual, multiply credits by 12
            }
        });
